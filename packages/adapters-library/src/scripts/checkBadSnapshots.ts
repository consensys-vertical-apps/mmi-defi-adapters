...
import { ChainIdToChainNameMap } from '../core/constants/chains.js'
...
export function checkBadSnapshots(
  program: Command,
    ...
    .action(async ({ protocols, products, key }) => {
      ...
      for (const { protocolId, productId } of allProducts) {
        ...
        const testCaseFileNames = testCases.map((testCase) => {
          return `${ChainIdToChainNameMap[testCase.chainId]}.${
            testCase.method
          }${testCase.key ? `.${kebabCase(testCase.key)}` : ''}.json`
        })
