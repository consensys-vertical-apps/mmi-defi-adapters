        ...
        try {
          for (const [chainId, db] of dbConnections) {
            for (const [protocolIdKey, supportedChains] of Object.entries(
              ...
              // ) {
              //   console.error({ chainId }, 'No provider found for chain')
...
function createDatabase(name: string) {
  try {
    ...
    if (fs.existsSync(dbPath)) {
      ...
      console.debug(`Database file does not exist: ${dbPath}`)
    }
    ...
    const tables = db
      ...
      .all()
    console.debug(`Tables in ${name} database:`, tables)

    return db
  } catch (error) {
    console.error(`Failed to create database or tables for '${name}':`, error)
    throw error
  }
}
